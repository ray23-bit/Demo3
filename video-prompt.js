 class VideoPromptGenerator {
    ...
    generatePrompt() {
        ...
        let subject = getValue('video-subject');
        let action = getValue('video-action');
        ...
        let color = getValue('video-color');
        let artists = getValue('video-artists');
        ...
        let details = getValue('video-details');
        let sound = getValue('video-sound');
        ...
        // Translate Indonesian input to English for the prompt (except character dialogues)
        if (this.currentLanguage === 'id') {
            ...
            // Translate subject
            subject = this.translateToEnglish(subject, translationMap);
            
            // Translate action
            action = this.translateToEnglish(action, translationMap);
            
            // Translate color
            color = this.translateToEnglish(color, translationMap);
            
            // Translate artists (keep original names)
            artists = artists.split(',').map(artist => {
                const trimmed = artist.trim();
                if (trimmed === 'Blade Runner') return 'Blade Runner';
                if (trimmed === 'Akira') return 'Akira';
                return this.translateToEnglish(trimmed, translationMap);
            }).join(', ');
            
            // Translate details
            details = this.translateToEnglish(details, translationMap);
            
            // Translate sound
            sound = this.translateToEnglish(sound, translationMap);
        }
        ...
        // Add characters and dialogue (keep original language for dialogues)
        if (charCount !== '0') {
            ...
            for (let i = 1; i <= charCount; i++) {
                ...
                if (charDesc) {
                    ...
                    const translatedDesc = this.currentLanguage === 'id' 
                        ? this.translateToEnglish(charDesc, {
                            'pria': 'man',
                            'paruh baya': 'middle-aged',
                            'janggut': 'beard',
                            'abu-abu': 'gray',
    ...
    // Helper method to translate Indonesian words to English
    translateToEnglish(text, translationMap) {
        if (!text) return '';
        
        // Split text into words and translate each if found in the map
        return text.split(' ').map(word => {
            const lowerWord = word.toLowerCase();
            return translationMap[lowerWord] || word;
        }).join(' ');
    }
